...
// ده كان الكود اللي في الفيديو بس كان في مشكلة في الالوان ف عدلته بالكود اللي تحت 
...
const Modal = ({ children, isOpen, onClose, title }) => {
    ...
    return (
        <div className="fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full h-[calc(100%-0.1rem)] max-h-full overflow-y-auto overflow-x-hidden bg-black/20 bg-opacity-50">
            <div className="relative p-4 w-full max-w-2xl max-h-full">
                {/* Modal content */}
                <div className="relative bg-white rounded-lg shadow-md">
                    ...
                    <div className="flex items-center justify-between p-4 md:p-5 border-b border-gray-300 rounded-t">
                        ...
                        <button
                            type="button"
                            className="text-gray-900 bg-transparent hover:bg-gray-100 hover:text-black rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center cursor-pointer"
                            onClick={onClose}
                        >
                            <svg
                                className="w-4 h-4"
                    ...
                    {/* Modal body */}
                    <div className="p-4 md:p-5 space-y-4 text-gray-800">
                        {children}
                    </div>
                </div>
            </div>
