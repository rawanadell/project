...
//@des Get all tasks (Admin: all,User:only assigned tasks)
//@route GET /api/tasks/
//@access Private
const getTasks = async(req,res)=>{
    try{
        ...
        const allTasks =await Task.countDocuments(
           ( req.user.role==="admin"|| req.user.role==="member")?{}:{assignedTo:req.user._id}
        );
...
const updateTaskChecklist=async(req,res)=>{
    ...
    if(!task.assignedTo.includes(req.user._id)&&(req.user.role!=="member")){
        return res
        .status(403)
